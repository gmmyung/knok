...
Graph bodies intentionally accept a small, static subset of Rust: typed tensor
arguments, `let` bindings, arithmetic and tensor ops, nested graph calls, and
one final expression. Axis, shape, and dtype choices are static and are checked
by the macro before IREE compilation.

Most shape-directed ops use either a target tensor type or const-generic axes:
...
See [docs/static-graph-syntax.md](docs/static-graph-syntax.md) for detailed
syntax rules and examples. In generated API docs, the same operation reference
lives under `knok::ops`.
...
| [docs/static-graph-syntax.md](docs/static-graph-syntax.md) | Detailed syntax accepted inside `#[knok::graph]` bodies. |
